...
const errorHandler = (err, req, res, next) => {
  ...
  if (err?.code === "P2025") {
    ...
    finalError = new BadRequestError("Foreign key constraint failed");
  } else if (err?.name === "PrismaClientValidationError") {
    finalError = new BadRequestError("Invalid input data types provided");
  }
  ...
  res.status(isOperational ? finalError.statusCode : 500).json({
    success: false,
    error: isOperational ? finalError.message : "Internal Server Error",
  });
};
